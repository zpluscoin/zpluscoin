...
// Copyright (c) 2011-2012 Zpluscoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("Zpluscoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ZpluscoinTEST0\r");
            Send(hSocket, "WHO #ZpluscoinTEST0\r");
        } else {
            // randomly join #Zpluscoin00-#Zpluscoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Zpluscoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Zpluscoin%02d\r", channel_number).c_str());
        }
