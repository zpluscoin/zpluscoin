...
uint256 hashGenesisBlock("0xf66c6a8a85083ba2630ee0fa9e8cb12781de81096fb1e10d31abab16808d35d4");
...
const string strMessageMagic = "Zpluscoin Signed Message:\n";
...
{
    // Zpluscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 6.66 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 6.67 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 6.67 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 6.67 * 2.8;
...
{
    // Zpluscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "zplus genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xd7, 0xc1, 0xa2, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZpluscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZpluscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZpluscoinMiner(CWallet *pwallet)
{
    printf("ZpluscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Zpluscoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZpluscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZpluscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZpluscoinMiner, pwallet));
}
